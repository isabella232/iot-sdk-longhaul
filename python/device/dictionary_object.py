...
"""
This module contains base classes which can be used for data objects that get serialized to and
from JSON.  It has the following characteristics:
    1. Unlike traditional Python objects, the attributes are locked once freeze_attributes() is
       called.  When the attributes are locked, any attempt to add a new attribute will raise
       an exception
    2. Data can be accessed by attribute name (e.g. obj.attribute_name) and can be converted to
       dict objects and serialized to JSON.  When converting to/from JSON, boxcar <--> camel
       conversion is automatic.  attribute_name gets serialized to json with the name i
       "attributeName"
    3. When serializing to JSON, attributes that have their default value are not included in the
       JSON.  Default values are set when freeze_attributes is called
"""


class SimpleObject(object):
    """
    Simple object which is able to be serialized as a sub-object, but does not contain
    the serialization code that a root object would have
    """
    ...
    def _get_attribute_names(self):
        ...
        """
        return [i for i in dir(self) if not i.startswith("_") and not callable(getattr(self, i))]
    ...
    def __setattr__(self, name, value):
        if (
            ...
            raise AttributeError("attribute {} does not exist on {}".format(name, type(self)))


class DictionaryObject(SimpleObject):
    """
    Object which represents a "root" object which is able to be serialized and possibly contains
    sub-objects (of type SimpleObject)
    """
        ...
        def fill_native_object_from_dict(native_object, dict_object):
            for dict_key in dict_object:
                ...
                if isinstance(dict_value, dict):
                    ...
                    raise ValueError(
                        "{} must be a dictionary, string, or scalar value".format(dict_key)
                    )
        ...
        def dict_from_native_object(native_object, default_object):
            ...
            for native_key in native_object._get_attribute_names():
                ...
                if _is_scalar(native_value):
                    ...
                    raise ValueError("{} must be a scalar value or an object".format(native_key))
            return dict_object
    ...
    def freeze(self):
        """
        freze attributes and set defaults.
        """
        self.freeze_attributes()
        if getattr(self.__class__, "_defaults", None) is None:
            self.__class__._defaults = True  # to prevent recursion on the next call
            self.__class__._defaults = self.__class__()
